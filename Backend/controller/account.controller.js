...
exports.getAllAccounts = async (req, res, next) => {
  const { createdAt, bankAccountNumber, type, userId, name, from, to } = req.query; // Extract query params
  ...
  try {
    ...
    // Apply filtering based on createdAt, from, and to
    if (from || to) {
      let startDate = from ? new Date(from) : new Date(0); // Default to the earliest possible date if 'from' is not provided
      let endDate = to ? new Date(to) : new Date(); // Default to the current date if 'to' is not provided
